    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for nextPos, f in newPosDist.items():
                allPossible[nextPos] += self.beliefs[p] * f
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while s < self.numParticles:
            for p in self.legalPositions:
                if s < self.numParticles:
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.legalPositions:
                allPossible[p] += emissionModel[util.manhattanDistance(p, pacmanPosition)] * belief[p]

            noNvalue = any(allPossible.values())
                    
            if noNvalue:
                l = []
                for i in range(0, self.numParticles):
                    l.append(util.sample(allPossible))
                self.particles = l
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            l.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, p))))
        self.particles = l
        "*** END YOUR CODE HERE ***"
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    d *= emissionModels[i][util.manhattanDistance(p[i], pacmanPosition)]
            allPossible[p] += d
